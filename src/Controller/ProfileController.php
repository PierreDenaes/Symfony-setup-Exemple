...
class ProfileController extends AbstractController
{
    ...
    #[Route('/profile/create-potion', name: 'profile_create_potion')]
    public function createPotion(Request $request): Response
    {   $user = $this->getUser();
        $profile = $user->getProfile();
        $potion = new Potion();
        $potion->setIdProfil($profile);
        $form = $this->createForm(PotionType::class, $potion);
        $form->handleRequest($request);
       
        if ($form->isSubmitted() && $form->isValid()) {
            $this->entityManager->persist($potion);
            $this->entityManager->flush();
            return $this->redirectToRoute('app_profile');
        }
        return $this->render('profile/create_potion.html.twig', [
            'form' => $form->createView()
        ]);
